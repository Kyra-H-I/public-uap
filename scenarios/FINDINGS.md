...
| F-004 | **Provider identity is not binding identity.** Two live windows/tabs of one application collide on one provider id; `ambiguous` covers rival providers, not twin sessions. | Browser walk; live field experience with two desktop editor windows. Two unrelated sources. | **Promoted + specified** (§1, *Provider identity is not binding identity*): `provider` stays the stable application identity; a *binding* is one live session, discriminated at transport level. Route by binding, grant by provider — the review caution against minting pseudo-provider identities is honored in the shape. Reference implementation in progress. |
