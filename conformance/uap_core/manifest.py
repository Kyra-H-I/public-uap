...
A **manifest** is routing metadata: who the provider is, what it is attached to,
which capability *ids* exist, and what safety features it supports. It is small,
cacheable by digest, and cheap enough to fetch on every session bind.
...
The other rule discovery enforces: **an action the provider cannot currently do is
absent, not present-and-failing** (spec §Manifest and capability discovery). Discovery
is shaped by the live
device, account, permission state, and flags — so a phone without calendar
permission simply has no calendar capability, rather than one that always errors.
"""
...
def is_valid_provider_id(value: str) -> bool:
    ...
    Public because the host MINTS ids as well as validating them: a desktop application is
    bound under an instance-qualified id (``com.example.editor.hazel-ridge``), and the code
    deciding whether a qualification still fits must ask the same question the constructor
    asks — not a second, drifting copy of this rule.
    """
...
class ProviderFeatures:
    ...
    events: bool = False
    """Emits ordered lifecycle events, so the host need not poll."""
...
class ActionDescriptor:
    ...
    """Everything this action changes. Empty means a pure read."""
...
class ProviderManifest:
    """Everything the host needs to ROUTE to a provider. Not a description of the app."""
    ...
    """Whether this is one session's capabilities or the deployment's public superset.

    Defaults to ``SESSION`` when absent, which is the safe direction: an unmarked
    document does not get to claim it is an unscoped superset.
    """
    ...
    def action_names(self) -> frozenset[str]:
        """Every action reachable on this provider right now."""
    ...
    def from_dict(cls, d: dict[str, Any]) -> ProviderManifest:
        ...
        self-award trust — the host assigns assurance from origin plus conformance
        evidence (the host's provider registry), so a compromised client cannot
        promote itself into the low-friction path by editing one JSON field.
        """
        try:
